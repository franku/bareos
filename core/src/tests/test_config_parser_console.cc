...
TEST(ConfigParser, test_console_dir_config_ucl_constructor)
{
  ...
  auto ucl_director = (ucl_imported.lookup("Director"));
  ASSERT_NE(ucl_director.type(), UCL_NULL);

  DirectorResource dir(ucl_director);
  std::cout << dir.resource_name_ << std::endl;
  std::cout << dir.password_.value << std::endl;
